...
namespace AzureServices.Services.Keyvault
{
    ...
    {
        public KeyvaultEmptySuccessResult(List<string> successMessages) : base(KeyVaultResultCode.NoContent, successMessages, default(T) ?? throw new Exception($"value {typeof(T)} is null"))
        {
        }
    }
    ...
    {
        public KeyvaultErrorResult(List<string> errorMessages, KeyVaultResultCode statusCode) : base(statusCode, errorMessages, default(T) ?? throw new Exception($"value {typeof(T)} is null"))
        {
        }
    }
